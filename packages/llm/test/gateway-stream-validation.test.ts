...
describe("LLM gateway stream validation", () => {
  ...
  const invalidFactories: ReadonlyArray<readonly [string, RawEventsFactory]> = [
    ["missing start", () => [{ type: "stream.finish", payload: { finishReason: "STOP" } }]],
    [
      "duplicate start",
      (_request, context) => [
        { type: "stream.start", payload: { callId: context.callId, providerId: "local", model } },
  ...
  it.each(invalidFactories)("rejects %s", async (_name, events) => {
    await expect(consume(gatewayWithEvents(events))).rejects.toBeInstanceOf(
      LLMInvalidResponseError,
    );
  });

  it.each(["call id", "provider id", "model"] as const)(
    "rejects mismatched start %s",
    async (name) => {
      await expect(
        consume(
