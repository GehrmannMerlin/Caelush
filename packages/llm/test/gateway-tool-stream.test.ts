...
describe("LLM gateway tool-call lifecycle", () => {
  ...
  const invalidToolFactories: ReadonlyArray<readonly [string, readonly unknown[]]> = [
    ["delta before start", [{ type: "tool_call.delta", payload: { toolCallId: "a", delta: "{}" } }]],
    ["completed before start", [{ type: "tool_call.completed", payload: { id: "a", name: "tool", input: {} } }]],
    ["duplicate start", [
    ...
    ["unfinished at finish", [{ type: "tool_call.start", payload: { toolCallId: "a", toolName: "tool" } }]],
