...
describe("tool output policy", () => {
  it("leaves content unchanged through the exact byte boundary", () => {
    ...
    expect(boundToolModelContent(content, { maxModelContentBytes: 5 })).toBe(content);
  });

  it("truncates at UTF-8 boundaries and states that output was truncated", () => {
    const content = "ab中文🙂cd".repeat(3);
    const bounded = boundToolModelContent(content, { maxModelContentBytes: 30 });
  ...
  it("returns a deterministic safe prefix when the marker cannot fit", () => {
    expect(boundToolModelContent("中文", { maxModelContentBytes: 3 })).toBe("中");
    expect(boundToolModelContent("中文", { maxModelContentBytes: 1 })).toBe("");
  });

  it("provides and validates the default policy", () => {
    expect(DEFAULT_TOOL_OUTPUT_POLICY).toEqual({ maxModelContentBytes: 64 * 1024 });
    expect(() => validateToolOutputPolicy({ maxModelContentBytes: 0 })).toThrow(
      /positive integer/i,
    );
  });
});
