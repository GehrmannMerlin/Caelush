...
describe("ProjectInstructionDiscovery", () => {
  it("loads instructions from project root to cwd with specific files later", async () => {
    ...
    const result = await new ProjectInstructionDiscovery(new LocalContextFileSystem()).discover(
      scope,
      root,
      cwd,
    );
  ...
  it("uses override, AGENTS, and CLAUDE fallback precedence per directory", async () => {
    ...
    await expect(
      new ProjectInstructionDiscovery(new LocalContextFileSystem()).discover(scope, root, cwd),
  ...
  it("fails closed for invalid UTF-8 and symlink escapes", async ({ skip }) => {
    ...
    await expect(
      new ProjectInstructionDiscovery(new LocalContextFileSystem()).discover(
        invalid.scope,
        invalid.root,
        invalid.cwd,
      ),
