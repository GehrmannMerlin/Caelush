...
it("builds a deterministic project intelligence snapshot without scanning source files", async () => {
  ...
  await writeFile(
    ...
    '{"name":"repo","packageManager":"pnpm@11.21.0","workspaces":["packages/*"],"scripts":{"test":"vitest"}}',
    "utf8",
  );
  await writeFile(
  ...
  await writeFile(
    path.join(workspace, "node_modules", "AGENTS.md"),
  ...
  expect(first.workspace.realRoot).toBe(
    await inspector
      .inspect({ workspace: input.workspace })
      .then((result) => result.workspace.realRoot),
  );
  ...
  expect(first.instructions.entries.map((entry) => entry.content)).toEqual([
    "root rules",
    "app rules",
  ]);
