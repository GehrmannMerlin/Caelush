...
it("builds real inspected and planned project context to an LLMRequest-ready message array", async () => {
  ...
  await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "fixture", packageManager: "pnpm@11.21.0" }), "utf8");
  await writeFile(path.join(root, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n", "utf8");
  await writeFile(path.join(root, "packages", "app", "package.json"), JSON.stringify({ name: "app" }), "utf8");
  await writeFile(path.join(root, "packages", "app", "AGENTS.md"), "Nested app rule", "utf8");
  await writeFile(path.join(root, "packages", "app", "src", "parser.ts"), "export function parse() { return true; }\n", "utf8");
  await writeFile(path.join(root, "packages", "app", "src", "parser.test.ts"), "test('parser', () => {});\n", "utf8");
  const workspace = { id: createWorkspaceId(), path: root };
  const snapshot = await createLocalProjectInspector().inspect({ workspace, cwd: path.join(root, "packages", "app") });
  ...
  const built = new ContextBuilder().build({
    ...
    history: [
      ...
      { role: "assistant", content: [{ type: "text", text: "I found the parser implementation." }] },
      { role: "user", content: "We also need to preserve the test behavior." },
      { role: "assistant", content: [{ type: "text", text: "Understood." }] },
    ],
    currentUserMessage: { role: "user", content: "Fix parser behavior without breaking the tests." },
  ...
  expect(request.messages.at(-1)).toEqual({ role: "user", content: "Fix parser behavior without breaking the tests." });
  expect(built.messages[0]?.role).toBe("system");
  expect(built.messages.some((message) => message.role === "user" && message !== built.messages.at(-1))).toBe(true);
  expect(built.messages[0]?.content).toContain("Root project rule");
  expect(built.messages[0]?.content).toContain("Nested app rule");
  expect(built.messages.some((message) => message.role === "user" && message.content.includes("parser.ts"))).toBe(true);
