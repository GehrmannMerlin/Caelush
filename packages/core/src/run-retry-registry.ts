...
export const DEFAULT_MAX_RETRY_TIMER_DELAY_MS = 2_147_483_647;

export interface RunRetryTimerHandle {
  cancel(): void;
}

export interface RunRetryTimerPort {
  schedule(delayMs: number, callback: () => void | Promise<void>): RunRetryTimerHandle;
}

export class SystemRunRetryTimer implements RunRetryTimerPort {
  schedule(delayMs: number, callback: () => void | Promise<void>): RunRetryTimerHandle {
...
interface Registration {
  ...
  handle: RunRetryTimerHandle | undefined;
}

export interface RunRetryRegistryOptions {
  readonly clock: { now(): TimestampMs };
  readonly timer?: RunRetryTimerPort;
  readonly maxDelayMs?: number;
  readonly onError?: (error: unknown) => void;
}

export class RunRetryRegistry {
  private readonly timer: RunRetryTimerPort;
  private readonly maxDelayMs: number;
  private readonly registrations = new Map<RunId, Registration>();

  constructor(private readonly options: RunRetryRegistryOptions) {
    this.timer = options.timer ?? new SystemRunRetryTimer();
    this.maxDelayMs = options.maxDelayMs ?? DEFAULT_MAX_RETRY_TIMER_DELAY_MS;
    if (!Number.isSafeInteger(this.maxDelayMs) || this.maxDelayMs <= 0) {
      throw new Error("Run retry timer maximum must be a safe positive integer.");
    }
  }
  ...
  arm(runId: RunId, nextAttemptAt: TimestampMs, callback: () => void | Promise<void>): void {
    this.disarm(runId);
  ...
  private wake(registration: Registration): void {
    ...
    const remaining = registration.nextAttemptAt - this.options.clock.now();
    if (remaining > 0) {
      this.schedule(registration);
      return;
    }
    ...
    void Promise.resolve()
      ...
      .catch((error: unknown) => {
        try {
          ...
          // A background retry wake must never create an unhandled rejection.
        }
      });
  }
}
