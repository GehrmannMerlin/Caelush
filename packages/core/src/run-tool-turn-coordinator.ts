 ...
 * ```
 *
 * The frozen request deliberately knows no workspace, no Runtime, no permission profile, no
 * approval policy and no resource policy. Every one of those is a fact about *this host and this
 * Run*, and the legacy `ToolBatchCoordinator` needs all of them — so they are **captured** here, at
 * the boundary that already holds them, rather than added to the general contract. A
 * `ToolTurnRequest` that carried a workspace would make every general Agent host describe one.
 *
 * The adapter owns the three host facts the frozen result cannot carry, and reports them through
 * the Core-private {@link RunToolTurnObservation} instead:
 *
 * ```text
...
/** What the adapter captured about one Run before any batch ran. */
export interface RunToolTurnContext {
   ...
   * ```
   *
   * This is Phase 7C's exactly-once protection, carried forward. A durable `RUNNING` invocation
   * must be recovered and never re-dispatched, and only the layer that knows *why* it is driving
   * can say which of the two this is — so it is threaded in explicitly rather than read off the
   * frozen directive, which says which batch is next and not whether its work already happened.
   */
...
export interface RunToolTurnDriverDependencies {
   ...
   *
   * A compatibility fallback for a `WAITING_TOOL_RESULTS` checkpoint written before the durable
   * policy existed. It is never consulted when the checkpoint carries one.
   */
 ...
 *
 * The returned function is the composition seam: the RunController hands it a durable snapshot and
 * the entry mode it already knows, and receives a coordinator that speaks the frozen Tool turn
 * contract over the legacy Tool System named by {@link RunToolTurnDriverDependencies.batches}.
 */
export function createRunToolTurnDriverFactory(
  ...
  return (snapshot, requestedMode) => {
    const context = captureRunToolTurnContext(dependencies, snapshot, requestedMode);
...
function captureRunToolTurnContext(
  dependencies: RunToolTurnDriverDependencies,
  ...
  return {
    ...
    observationPolicy: resolveToolObservationPolicy(continuation, dependencies),
    ...
    environment: { workspace: snapshot.run.workspace, runtime: snapshot.run.runtime },
    securityContext: createToolSecurityContext(snapshot.run, state),
  };
}
 ...
 * ```
 *
 * A checkpoint that carries a policy is projected under exactly that policy, so a restart that
 * changed the Context configuration cannot retroactively re-truncate a batch the model already
 * asked for.
 */
function resolveToolObservationPolicy(
  ...
  return dependencies.hostObservationPolicy?.() ?? defaultObservationPolicy();
}
...
async function executeRunToolTurn(
  ...
  if (admission.kind === undefined) {
    // No resource policy is configured for this Run. Nothing was decided, so nothing is recorded:
    // an observation that named a decision nobody made would be a fact about work that never ran.
  } else {
 ...
 *
 * The `HARD_STOP` arm carries the exact dimension, accounted count and limit the Tool budget
 * authority reported, so the budget settlement reports them rather than re-deriving them.
 */
...
async function admitResource(
  ...
  // The count is read from the state the decision was made against, which is the same durable
  // number `WAITING_RESOURCE` has always persisted. The frozen `RESOURCE_WAIT` result carries a
  // reason and nothing else, so this observation is its only authority.
  const replanCount = (await repository.get(context.run.id))?.replanCount ?? state.replanCount;
  switch (decision.kind) {
    case "HARD_STOP":
      // The exact numbers the Tool budget authority returned, reported and never re-derived.
      return {
        kind: "HARD_STOP",
        replanCount,
  ...
  switch (outcome.kind) {
    ...
    case "WAITING_APPROVAL":
      return settleObservation(observation, "WAITING_APPROVAL", outcome.completedResults, () => ({
        ...
        // complete, so the model is shown none of it. The completed invocations are already durable
        // in the Tool ledger, and that — not a partial model message — is what recovery resumes
        // from, so an approved trailing call can never cause a completed one to run twice.
        completedResults: [],
    ...
    case "BUDGET_EXCEEDED":
      return settleObservation(observation, "BUDGET_EXCEEDED", outcome.completedResults, () => ({
        kind: "BUDGET_EXCEEDED",
        completedResults: [],
        // The exact numbers the Tool budget authority returned. They are reported, never
        // re-derived: a Run Layer that recomputed them would be a second accounting authority.
 ...
 *
 * The observation is the only place these facts exist: the frozen result reports what the model
 * will be told, and `executionAttempted` distinguishes a batch that ran from one that never reached
 ...
 * ```
 *
 * It runs **after** the Run continuation is durable, and that ordering is the point: resource
 * progress accounts for work the Run *accepted*, so a settlement whose commit lost its compare-and
 * swap must not have advanced it. The durable Tool invocations remain the recovery authority, and
 * a later recovery reads them instead of replaying a handler.
 */
export async function recordRunToolTurnProgress(input: {
  ...
  const policy = input.context.run.resourcePolicy;
  if (repository === undefined || policy === undefined) return;
  const current = await repository.createOrGet(input.context.run.id, {
    policyVersion: "adaptive-resource-governance.v1",
    mode: policy.mode,
    now: input.dependencies.clock.now(),
  });
  const requestFingerprint = fingerprintToolBatch(input.context.pendingDecision.toolRequests);
  ...
  await updateResourceState(input.dependencies, current, {
    ...current,
    agentTurnsConsumed: input.state.usage.steps,
    toolOperationsConsumed: input.state.usage.toolCalls,
