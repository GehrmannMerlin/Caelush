...
describe("architecture v2 rule engine", () => {
  it("keeps every frozen rule id unique and derived from its edge", () => {
    ...
    expect(rules.RULE_IDS).toContain("STORAGE_MUST_NOT_DEPEND_ON_WEB");
  ...
  it("resolves a rule only for a forbidden direction and kind", () => {
    ...
    expect(rules.findRule("source-import", "storage", "ai")).toBeUndefined();
  ...
  it("declares both a source and a manifest rule for each forbidden edge", () => {
    ...
    const sourceEdges = new Set(
      rules.DEPENDENCY_RULES.filter((rule) => rule.kind === "source-import").map(edgeOf),
    );
    const manifestEdges = new Set(
      rules.DEPENDENCY_RULES.filter((rule) => rule.kind === "package-manifest").map(edgeOf),
    );
...
describe("architecture v2 boundary evaluation", () => {
  it("passes a workspace whose imports follow the allowed direction", async () => {
  ...
  it("fails on a forbidden source import and names the rule and package", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: { "index.ts": 'import { shell } from "@caelush/runtime";\nexport {};\n' },
    ...
    expect(failed.output).toContain("Location:\npackages/agent/src/extra.ts:1:27");
  ...
  it("normalizes a deep subpath import to one package edge", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: {
    ...
    expect(evaluated.violations[0]?.targetPackage).toBe("runtime");
    expect(evaluated.violations[0]?.specifier).toBe("@caelush/runtime");
    expect(evaluated.violations[0]?.detail.occurrenceCount).toBe(3);
    expect(evaluated.violations[0]?.detail.rawSpecifier).toBe("@caelush/runtime/deep/one");
  });

  it("detects dynamic import and export-from boundaries", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: {
    ...
    const evaluated = boundaries.evaluateScan(await scanner.scanWorkspace(workspace.root));
    const ruleIds = evaluated.violations.map((violation) => violation.rule).sort();
    ...
    const storage = evaluated.violations.find((violation) => violation.targetPackage === "storage");
    expect(storage?.detail.importKinds).toEqual(["export-from", "dynamic-import"]);
  });

  it("detects an illegal workspace dependency in package.json without source imports", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/ai": {
  ...
  it("scans every manifest dependency section", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/client": {
          source: { "index.ts": "export {};\n" },
  ...
  it("keeps the Web and CLI hosts off the kernel and persistence packages", async () => {
    const hostSource = [
      'import { openStore } from "@caelush/storage";',
  ...
  it("does not flag a project for importing itself through a deep subpath", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
  ...
  it("separates diagnostic test-scope edges from the authoritative source scope", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "apps/web": {
          source: { "index.ts": "export {};\n" },
...
describe("architecture v2 legacy baseline ratchet", () => {
  it("freezes an existing violation when it is present in the baseline", async () => {
  ...
  it("fails on a new violation that is absent from the baseline", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
  ...
  it("fails with a stale baseline entry after the violation disappears", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
  ...
  it("keeps the baseline stable when a baselined file is edited or gains another import", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
    ...
    await writeFile(
      ...
      [
        ...
        'import { shell } from "@caelush/runtime/exec";',
  ...
  it("detects baseline drift even while the frozen violations still match", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: {
  ...
  it("reports a missing baseline instead of silently passing", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
  ...
  it("rejects a malformed baseline document", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await writeFile(
      baselinePathOf(workspace.root),
  ...
  it("refuses a baseline write in CI when it would add unreviewed entries", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
    const before = await readFile(baselinePathOf(workspace.root), "utf8");
    ...
    expect(result.exitCode).toBe(1);
    expect(result.output).toContain("cannot be regenerated in CI");
    expect(result.output).toContain("would add entries");
    expect(await readFile(baselinePathOf(workspace.root), "utf8")).toBe(before);
  });

  it("allows a CI baseline write that only shrinks the baseline", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
  ...
  it("refuses to create a missing baseline from CI while violations exist", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
  ...
  it("records reproducible provenance in the baseline", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });

    const first = await readFile(baselinePathOf(workspace.root), "utf8");
    const document = JSON.parse(first);
    expect(document.generatedFromHead).toBe(FIXED_HEAD);
    expect(document.generatedAt).toBe(FIXED_TIME.toISOString());

    await check(workspace.root, { writeBaseline: true });
    expect(await readFile(baselinePathOf(workspace.root), "utf8")).toBe(first);
  });

  it("never rewrites the baseline during an ordinary check", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
    const before = await readFile(baselinePathOf(workspace.root), "utf8");
  ...
  it("produces a deterministic, sorted, one-entry-per-line baseline", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: {
  ...
  it("round-trips a rendered baseline document through the parser", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    const scan = await scanner.scanWorkspace(workspace.root);
  ...
  it("detects duplicate baseline entries", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
    const document = JSON.parse(await readFile(baselinePathOf(workspace.root), "utf8"));
  ...
  it("fails a new violation and a stale entry in the same run", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    await check(workspace.root, { writeBaseline: true });
      ...
      violations: [
        {
          ...
          line: 99,
          column: 7,
          detail: { rawSpecifier: "@caelush/runtime/exec" },
        },
      ],
      sourceEdges: 1,
      manifestEdges: 0,
...
describe("architecture v2 repository integration", () => {
  it("ships a checked-in baseline that matches the current checkout", async () => {
  ...
  it("parses the real source scope and resolves every Caelush specifier", async () => {
  ...
  it("keeps the checked-in baseline deterministic and reviewed", async () => {
  ...
  it("exposes a working read-only command-line entry point", async () => {
    const { stdout } = await execFileAsync(process.execPath, [CHECKER_PATH, "--verify-baseline"], {
      cwd: repositoryRoot,
    });
  ...
  it("fails the command-line entry point when a forbidden edge is injected", async () => {
    const workspace = await fixture(
      v2WorkspaceSpec({
        "packages/agent": {
          source: { "index.ts": 'import { shell } from "@caelush/runtime";\nexport {};\n' },
  ...
  it("refuses a CI baseline write from the command line when it would grow the baseline", async () => {
    const workspace = await fixture(legacyWorkspaceSpec());
    const baseline = baselinePathOf(workspace.root);
    await check(workspace.root, { writeBaseline: true });
    ...
    await expect(
      ...
      code: 1,
      stdout: expect.stringContaining("cannot be regenerated in CI"),
    });
  });

  it("rejects an unknown command-line argument with the usage message", async () => {
    await expect(
