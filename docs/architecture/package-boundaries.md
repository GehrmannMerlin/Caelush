...
本文档冻结 Caelush V1 Phase 0 的工程边界。它描述未来职责，不代表 Phase 0 已实现对应功能。
...
- `protocol` 位于最底层，只承载跨边界 Contract，不依赖其他 Caelush feature package。
...
| `@caelush/core`          | RunController、AgentLoop、AgentState 协调、SessionManager、RunExecutionContext、Retry、Budget                               | 具体 LLM、具体 Tool、文件/进程执行、权限实现、UI、数据库细节 | `protocol`、`events`、`llm`、`context`、`tools`、`security`、`verification`、`shared` | apps、具体 provider/tool/runtime 实现的反向耦合   | daemon、CLI adapter、Web adapter               |
...
Phase 1 会首先正式化 `protocol` 中的 Contract，然后由 Core、Events、Storage 和 adapters 逐步消费它。保持边界现在就可验证，可以避免后续把 CLI/Web 逻辑、provider 细节或执行权限渗入 Kernel。
