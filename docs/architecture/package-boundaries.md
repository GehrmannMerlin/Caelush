...
| `@caelush/storage`       | SQLite/Drizzle schema、Protocol entity repositories、Run State Snapshot、Durable Event Store adapter、migration/lifecycle boundary | AgentLoop、权限、HTTP、UI、Provider、EventBus 消费编排                    | `protocol`、`events`、`shared`                                                        | apps、Core 内部状态机实现、直接执行 Tool、Events 反向依赖 Storage | daemon、observability、composition root                |
...
Phase 1 已正式化 `protocol` 中的 Contract，并由 Core 的状态机通过公共入口消费它；Phase 2 通过 `events → protocol` 与 `storage → events + protocol` 接入持久化和事件消费。Phase 5C 允许 `context → @caelush/llm/messages` 复用 provider-independent message contract，但不允许 Context 触达 LLM runtime 或 provider adapter。`events` 只拥有 port，不反向依赖 `storage`；`storage` 也不依赖 `core`，而是保存 Protocol entity 和 Durable Event。保持边界现在就可验证，可以避免后续把 CLI/Web 逻辑、provider 细节或执行权限渗入 Kernel。
