...
Phase 1 已正式化 `protocol` 中的 Contract，并由 Core 的状态机通过公共入口消费它；Phase 2 通过 `events → protocol` 与 `storage → events + protocol` 接入持久化和事件消费。`events` 只拥有 port，不反向依赖 `storage`；`storage` 也不依赖 `core`，而是保存 Protocol entity 和 Durable Event。保持边界现在就可验证，可以避免后续把 CLI/Web 逻辑、provider 细节或执行权限渗入 Kernel。
