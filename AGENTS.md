...
## Phase 11A Verification Domain, Planning and Evidence Contract Rules

- Phase 11 contains exactly 11A, 11B, 11C, and 11D. This round implements only 11A. Do not add 11A-1, 11A-2, 11E, or another Verification round.
- Phase 11A creates intent-only `VerificationPlan`/`VerificationCheck`/`VerificationEvidence` contracts and a durable Final Candidate → `VERIFYING` boundary. It does not execute lint, typecheck, tests, builds, Git/workspace checks, task acceptance, an LLM reviewer, or any Verification runner; it does not transition `VERIFYING` to `COMPLETED`.
