...
- Session history is daemon-owned. Only prior same-Session, exact-same-workspace, `COMPLETED` Runs with valid `VerifiedRunFinalResult` and `finishedAt <= currentRun.createdAt` are eligible; order is `createdAt ASC`, then Run ID `ASC`, with a newest suffix bounded by `MAX_SESSION_HISTORY_RUNS = 100`.
- Each eligible prior Run projects only `goal` as a user message and verified final `text` as an assistant message. Tool calls/results, stdout/stderr, patches, reasoning, provider payloads, credentials and host paths never enter the prefix or CLI transcript. `ContextBuilder` remains the only token authority.
- Core receives the synthetic prefix through `RunExecutionConfig.historyPrefix`. Fresh/retry/recover/repair execution removes the current open Run turn before `AgentLoop.run()` adds the current goal; Tool-result resume prepends the prefix to the complete current turn exactly once. The prefix is never appended to durable Conversation.
- CLI public transcript entries are limited to user prompts, validated `VerifiedRunFinalResult.text`, and bounded terminal notices. Valid detail events are ignored for transcript purposes; mismatched Run IDs are ignored; completion is never inferred from model text or event payloads.
- A stream failure is transport state, not a durable Run failure. Phase 12B has no automatic reconnect, resume, cancellation, Approval interaction, timeline/detail UI, daemon auto-start, or packaging. `dispose()` aborts only the local stream; an active daemon Run may continue.

## Phase 12C Live Agent Timeline and Process Visualization Rules

- Phase 12C owns only live Agent Timeline projection and visualization. Phase 12 remains exactly 12A, 12B, 12C, 12D and 12E; do not add 12C-1, 12C-2, 12F or implement 12D/12E capabilities.
- AgentEvent remains the source of CLI execution activity. The CLI must not invent Agent work with timers, mock reasoning, fake plans, fake Tool calls, fake file operations or fake verification progress.
- Timeline state is a bounded product projection and must never become execution authority or write back to Run, AgentState, ToolInvocation, ToolObservation, Verification, Storage or EventBus.
- Only `USER_VISIBLE` AgentEvents may be rendered in the normal user timeline. `DEBUG`/`SYSTEM`, provider payloads, system prompts, raw hidden reasoning and raw chain-of-thought must not be rendered; `reasoning.summary` is the only normal reasoning surface.
- `displayHistory` is the single ordered public history for USER, settled Timeline entries, verified ASSISTANT text and Run terminal notices. Settled history uses one main Ink `<Static>`; active Tool, Plan, Verification, Process, Approval and Retry work remains dynamic.
- Tool Timeline correlation must use durable `invocationId`; never use a latest-Tool or visual-order heuristic. File events may attach to an active Tool only when same-step compatibility is proven and exactly one candidate exists; otherwise they remain standalone.
- Raw `ToolInvocation.args`, raw `apply_patch` input, credentials, Observation internals, evidence/seal data and exception text must never enter public Timeline state or user-visible events.
- Public Tool presentation is a data-only port defined in `packages/tools` and implemented by `packages/security`; `packages/tools` must not depend on Security. Presentation failure falls back to a generic label and cannot affect Tool execution or durable settlement.
- Public command and Tool result presentations must reuse the existing Secret Redaction, sensitive-path and Tool Result sanitizer policies. Raw `exec_command` strings must not enter user-visible events without that boundary; `write_stdin.chars`, password input, stdin content, environment variables and authorization headers must never be shown.
- Terminal-controlled output must be sanitized for OSC, CSI, cursor/title/hyperlink escape sequences and unsafe C0 controls before display, while preserving newline, tab and normal Unicode. All presentation text/output must have explicit UTF-8 byte and entry bounds with visible omission markers.
- File presentation may show workspace-relative per-file `A/M/D/R` summaries and additions/deletions. Line-level diff is allowed only from a run-attributed, sanitized, bounded source; do not reconstruct a workspace-wide diff or print raw patch arguments.
- Timeline reducers must be pure, deterministic and replayable. Exact duplicate events cannot create duplicate entries; conflicting durable sequence identities fail closed; ephemeral events never advance the durable replay cursor; old Run events cannot contaminate a new Run.
- Long-running processes must not block unrelated settled history. A terminal Run flushes presentation-only active work before the canonical verified final assistant response is appended; it must not mark a durable ToolInvocation failed or fabricate a process stop.
- Phase 12C may display real approval notices and stop at `WAITING_APPROVAL`, but does not implement approval resolution UI, cancellation UX, Ctrl+C cancellation, Session resume/picker, automatic SSE reconnection, daemon auto-launch, non-interactive/pipe mode, Web UI, MCP, Browser, Computer Use, Sub-Agent or Multi-Agent capabilities.
...
- Approval UI must refresh `listPendingApprovals(runId)`, use the exact `approval.id`, default to `REJECT`, and map only to `{action: "APPROVE", scope: "ONCE"|"RUN"}` or `{action: "REJECT"}`. It may show only bounded safe title/reason/risk/tool/capability/summary fields; raw action JSON, command/patch/stdin, keys, credentials, provider payloads and exception text are forbidden.
- Approval resolution has one in-flight operation per Approval ID and uses a control generation to ignore stale responses. Conflicts reconcile from canonical pending approvals and Run state; the CLI never guesses another Approval or grants a local permission.
- Active Run `Ctrl+C` sends one `cancelRun(runId)` call and shows `CANCELLING` until canonical `response.run.status` or a terminal event confirms settlement. `SETTLED` alone is not `CANCELLED`; HTTP failure must not create a fake terminal state. Cancellation remains available for `RUNNING`, `WAITING_APPROVAL`, `VERIFYING`, and an active disconnected representation.
- `Ctrl+D` aborts only the local stream, invalidates the stream generation, clears local attachment and reports that the daemon Run continues. It never calls `cancelRun()` and never changes durable Run state.
- `caelush` starts a new Session; `--continue`/`-c` selects the latest matching current-workspace Session; `--resume`/`-r` opens a bounded picker; `--resume <session-id>` performs exact resume. Parser errors, workspace mismatches, legacy ambiguity and no-match cases fail safely without creating a replacement Session or changing cwd.
- Continue candidates are bounded to 100 Sessions, enriched with at most one newest Run at concurrency 8, filtered by normalized exact workspace, and sorted by derived Run activity (`finishedAt ?? startedAt ?? createdAt ?? session.updatedAt`) then Session ID. Resume history is bounded to 100 Runs, ordered `createdAt ASC`/ID, and contains only goals, valid verified final text and bounded terminal notices.
- Resuming a Session uses its full `defaultWorkspace` identity and current daemon default Run configuration. Non-terminal Runs are limited to `PENDING`, `RUNNING`, `WAITING_APPROVAL` and `VERIFYING`; multiple candidates require a recovery picker, `PENDING` requires explicit start confirmation, and active Runs attach the event stream before at most one `recoverRun()` per generation.
